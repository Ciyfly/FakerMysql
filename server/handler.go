...
func NewHandler(e *sqle.Engine, sm *SessionManager, rt time.Duration, connMessage *ylog.ConnMessage) *Handler {
	return &Handler{
		e:           e,
		sm:          sm,
		readTimeout: rt,
...
func (h *Handler) NewConnection(c *mysql.Conn) {
	// logrus.WithField(sqle.ConnectionIdLogField, c.ConnectionID).Debugf("NewConnection")
	// h.connMessage.ToMessage(&ylog.Message{
	// 	Type: ylog.CONNECT,
	// })
}
